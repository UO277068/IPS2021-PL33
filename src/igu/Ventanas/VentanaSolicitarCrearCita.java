...
public class VentanaSolicitarCrearCita extends JDialog {
			...
			chckbxEspecialidad.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent e) {
					...
					if (chckbxEspecialidad.isSelected()) {
						...
						btnAñadirMedico.setText("A�adir especialidad");
						...
						btnAñadirMedico.setText("A�adir medicos");
			...
			comboBoxHoraInicioCita.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent e) {
					if (comboBoxHoraInicioCita.getSelectedIndex() < 0) {
						...
						getComboBoxHoraInicioDisponibilidad()
								.setSelectedIndex(comboBoxHoraInicioCita.getSelectedIndex());
					}
		...
		if (btnHorariosMedicos == null) {
			...
			btnHorariosMedicos.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					getComboBoxHoraInicioDisponibilidad()
							.setSelectedIndex(getComboBoxHoraInicioCita().getSelectedIndex());
					getComboBoxMinutoInicioDisponibilidad()
							.setSelectedIndex(getComboBoxMinutoInicioCita().getSelectedIndex());

					getComboBoxHoraFinalDisponibilidad().setSelectedIndex(getComboBoxHoraFinCita().getSelectedIndex());
					getComboBoxMinutoFinalDisponibilidad()
							.setSelectedIndex(getComboBoxMinutoFinCita().getSelectedIndex());
					//
			...
			textFieldFiltroMedico.addKeyListener(new KeyAdapter() {
				...
				public void keyReleased(KeyEvent e) {
					if (!chckbxEspecialidad.isSelected()) {
						...
						String[] especialidadesFiltradas = filtrarEspecialidades(especialidades,
								getTextFieldFiltroMedico().getText());
		...
		if (paciente != null) {
			if (checkValoresVentanaPrincipal()) // Comprueba que se han elegido los valores minimos para crear una cita
			{
				...
				if (getListSeleccionados().getModel().getSize() != 0) { // Medicos seleccionados?
					...
					respuesta = JOptionPane.showConfirmDialog(null, "�Quiere solicitar una cita sin medicos definidos?",
							"Advertencia al Crear la cita", JOptionPane.YES_NO_OPTION);
	...
	private Integer GeneraCita(PacienteDto paciente, Integer respuesta, boolean urge, MedicoDto medico) {
		...
		if (medico != null) {
			...
			if (!compruebaJornada(medico.id)) {
				respuesta = JOptionPane.showConfirmDialog(
						null, "El horario de la cita no forma parte de la jornada laboral del medico " + medico.name
								+ " " + medico.surname + ".",
						"Advertencia al Crear la cita", JOptionPane.YES_NO_OPTION);
			}
		...
		if (respuesta == JOptionPane.YES_OPTION) {
			...
			// solicitud.cuerpo+="No determinandas" + "#"; //A�adir causas una vez el medico
			// las determine
...
//		 cita.idSala=Sala;
			...
			// cita.preescripcion="";
			// cita.contacto=paciente.contacto; //Por defecto contacto de la cita = contacto
			// del paciente
	...
	private boolean compruebaHoraSala(CitaDto cita) {
		...
		for (CitaDto c : listaDeCitas) {
			if (Timestamp.valueOf(c.horaInicio).after(Timestamp.valueOf(cita.horaInicio))
					|| (Timestamp.valueOf(c.horaInicio).before(Timestamp.valueOf(cita.horaInicio))
							&& Timestamp.valueOf(c.horaFinal).after(Timestamp.valueOf(cita.horaFinal)))) {
				int respuesta = JOptionPane.showConfirmDialog(null,
						"La sala " + cita.idSala + "esta ocupada en ese momento.�Desea crear la cita?",
						"Advertencia al Crear la cita", JOptionPane.YES_NO_OPTION);
				if (respuesta == JOptionPane.YES_OPTION) {
					return true;
				} else {
	...
	private boolean checkValoresVentanaPrincipal() {
		if (!checkCamposVacios()) {
			...
			JOptionPane.showInternalMessageDialog(null, "Escoja una fecha para establecer la cita");
			return false;
		}
//		if(!checkHoraFecha()) {
...
//	}
		...
		if (textFieldFiltroMedicosDisponibilidad == null) {
			...
			textFieldFiltroMedicosDisponibilidad.addKeyListener(new KeyAdapter() {
				...
				public void keyReleased(KeyEvent e) {
					if (!chckbxFiltroMedicosDisponibilidad.isSelected()) {
						String[] medicosstr = medicosToString(
								filtrarListaMedicos(medicos, textFieldFiltroMedicosDisponibilidad.getText()));
						ListModel<String> model = new DefaultComboBoxModel<String>(medicosstr);
						listMedicos.setModel(model);
					} else {
						String[] seleccionadosFiltrados = medicosToString(filtrarListaMedicos(medicosSeleccionados,
								textFieldFiltroMedicosDisponibilidad.getText()));
				...
				public void actionPerformed(ActionEvent e) {
					if (checkCamposVaciosDisponibilidad()) {
						...
						JOptionPane.showInternalMessageDialog(null,
								"A�ada la fecha y hora de entrada y de salida por la que desea buscar horarios");
					}
	...
	private List<String> obtenHorariosMedicosDisponibilidad() {
		...
		if (getListMedicosDisponibilidad().getSelectedIndex() != -1) {
			...
			List<JornadaDto> jornadas = new ListJornadaLaboralByMedicoAction(
					medicosSeleccionadosDisponibilidad.get(0).id).execute();
				...
				if (medico.id != (medicosSeleccionadosDisponibilidad.get(0).id)) {
					...
					for (JornadaDto j : jornadasC) {
						for (JornadaDto jornada : copia) {
							...
							} else if ((inicioFecha.before(inicioj) || igualdad1)
									&& (finFecha.before(finj) || igualdad2)) {
							...
							} else if ((inicioFecha.after(inicioj) || igualdad1)
									&& (finFecha.after(finj) || igualdad2)) {
...
//	        }
	...
	private boolean checkCamposVaciosDisponibilidad() {
		if (getComboBoxHoraFinalDisponibilidad().getSelectedIndex() == -1
				&& getComboBoxHoraInicioDisponibilidad().getSelectedIndex() == -1
				&& getComboBoxMinutoFinalDisponibilidad().getSelectedIndex() == -1
				&& getComboBoxMinutoInicioDisponibilidad().getSelectedIndex() == -1
