...
public class VentanaSolicitarCrearCita extends JDialog {
		...
		if (paciente != null) {
			if (checkValoresVentanaPrincipal()) // Comprueba que se han elegido los valores minimos para crear una cita
			{
				...
				if (getListSeleccionados().getModel().getSize() != 0) { // Medicos seleccionados?
					...
					respuesta = JOptionPane.showConfirmDialog(null, "ï¿½Quiere solicitar una cita sin medicos definidos?",
	...
	private Integer GeneraCita(PacienteDto paciente, Integer respuesta, boolean urge, MedicoDto medico) {
		...
		if (respuesta == JOptionPane.YES_OPTION) {
			...
			// solicitud.cuerpo+="No determinandas" + "#"; //Aï¿½adir causas una vez el medico
			// las determine
		...
		for (CitaDto c : listaDeCitas) {
			if (Timestamp.valueOf(c.horaInicio).after(Timestamp.valueOf(cita.horaInicio))
				...
				int respuesta = JOptionPane.showConfirmDialog(null,
						"La sala " + cita.idSala + "esta ocupada en ese momento.ï¿½Desea crear la cita?",
						"Advertencia al Crear la cita", JOptionPane.YES_NO_OPTION);
				if (respuesta == JOptionPane.YES_OPTION) {
					return true;
				} else {
				...
				public void actionPerformed(ActionEvent e) {
					if (checkCamposVaciosDisponibilidad()) {
						...
						JOptionPane.showInternalMessageDialog(null,
								"Aï¿½ada la fecha y hora de entrada y de salida por la que desea buscar horarios");
					}
